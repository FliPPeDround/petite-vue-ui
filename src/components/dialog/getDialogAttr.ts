export const getDialogAttr = () => {
  ...
  for (let i = 0; i < pvDialogAttr.length; i++) {
    ...
    const titleSlot = pvDialogAttr?.[i].children?.['title' as unknown as number]?.outerHTML.slice(21, -11).replace(/[\r\n]/g, "")
    const footerSlot = pvDialogAttr?.[i].children?.['footer' as unknown as number]?.outerHTML.slice(22, -11).replace(/[\r\n]/g, "")
